...
export const Canvas: React.FC<CanvasProps> = ({
  ...
  const startTextInput = useCallback((point: Point, existingElement?: Element) => {
    console.log('🟢 startTextInput called:', { point, existingElement: existingElement?.id });
    setTextPosition(point);
    setTextInput(existingElement?.text || '');
    setEditingTextId(existingElement?.id || null);
    setIsTextActive(true);
    console.log('🟢 Text input state set, position:', point);
    
    // Focus the input after state update with multiple attempts
    const focusAttempts = [100, 200, 300]; // Try multiple times
    focusAttempts.forEach((delay) => {
      setTimeout(() => {
        if (textInputRef.current && document.contains(textInputRef.current)) {
          console.log(`🟢 Focusing textarea element (attempt after ${delay}ms)`);
          textInputRef.current.focus();
          textInputRef.current.select();
        } else {
          console.log(`🔴 textInputRef.current is null or not in DOM (after ${delay}ms)`);
        }
      }, delay);
    });
  }, []);
  ...
  const finishTextInput = useCallback(() => {
    console.log('🟡 finishTextInput called, textInput:', textInput);
    if (!textInput.trim()) {
      console.log('🟡 No text entered, cleaning up');
      // If no text entered, remove the element if it was just created
      if (!editingTextId) {
        // Remove the last created text element if it's empty
        const lastElement = elements[elements.length - 1];
        if (lastElement && lastElement.type === 'text' && !lastElement.text) {
          console.log('🟡 Removing empty text element:', lastElement.id);
    ...
    console.log('🟡 Processing text input, editingTextId:', editingTextId);
    if (editingTextId) {
      // Update existing text element - keep current size, don't auto-resize
      console.log('🟡 Updating existing element with text:', textInput);
      const updatedElements = elements.map(el => 
        el.id === editingTextId ? { 
          ...el, 
          text: textInput
          // Keep existing width and height - user controls size via resizing
        } : el
      );
      onElementsChange(updatedElements);
    } else {
      // Create new text element with initial sizing
      const lines = textInput.split('\n');
      const lineHeight = fontSize * 1.2;
      const textWidth = Math.max(...lines.map(line => line.length * fontSize * 0.6), 100);
      const textHeight = Math.max(lines.length * lineHeight + 16, 40); // 16px padding
      
      console.log('🟡 Creating new text element with text:', textInput);
      const newElement: Element = {
        id: generateId(),
        type: 'text',
        x: textPosition.x,
        y: textPosition.y,
        width: Math.min(textWidth + 16, 400), // Max width 400px
        height: textHeight,
        strokeColor: '#374151',
    ...
    console.log('🟡 Finishing text input, cleaning up state');
    setIsTextActive(false);
    setEditingTextId(null);
    setTextInput('');
  }, [textInput, editingTextId, elements, onElementsChange, textPosition, fontSize, fontFamily]);
  ...
  const handleMouseDown = useCallback((event: React.MouseEvent) => {
    ...
    // NEW TEXT TOOL LOGIC - create text box like code block
    if (selectedTool === 'text') {
      console.log('🔵 Text tool clicked at point:', point);
      ...
      if (clickedTextElement) {
        console.log('🔵 Editing existing text element:', clickedTextElement.id);
        // Edit existing text
        startTextInput(point, clickedTextElement);
        onSelectionChange([clickedTextElement.id]);
      } else {
        console.log('🔵 Creating new text element at:', point);
        // Create new text box immediately (like code block)
        const newElement: Element = {
          id: generateId(),
          type: 'text',
          x: point.x,
          ...
          width: 200,
          height: 60,
          strokeColor: '#374151', // Visible border
          backgroundColor: 'rgba(243, 244, 246, 0.9)', // Light gray background like code block
          strokeWidth: 2,
          strokeStyle: 'solid',
          roughness: 0,
          opacity: 1,
        ...
        console.log('🔵 New text element created:', newElement.id);
        onElementsChange([...elements, newElement]);
        onSelectionChange([newElement.id]);
        
        // Start editing the new text box
        startTextInput(point, newElement);
      }
      return;
    }
  ...
  // Debug effect for text input state
  useEffect(() => {
    console.log('🔍 Text input state changed:', {
      isTextActive,
      textPosition,
      textInput,
      editingTextId,
  ...
  }, [isTextActive, textPosition, textInput, editingTextId, zoom]);

  // Effect to handle textarea focus when it becomes active
  useEffect(() => {
    if (isTextActive && textInputRef.current) {
      console.log('🟢 useEffect: Textarea is active, attempting to focus');
      const textarea = textInputRef.current;
      
      // Use requestAnimationFrame to ensure DOM is ready
      requestAnimationFrame(() => {
        if (textarea && document.contains(textarea)) {
          console.log('🟢 useEffect: Focusing textarea via requestAnimationFrame');
          textarea.focus();
          textarea.select();
        }
      });
  ...
  const renderElement = (element: Element) => {
    ...
    switch (element.type) {
      ...
      case 'text': 
        // Calculate wrapped text lines based on element width with better accuracy
        const textContent = element.text || '';
        const fontSize = element.fontSize || 16;
        const characterWidth = fontSize * 0.6; // Approximate character width
        const maxCharsPerLine = Math.floor((element.width - 16) / characterWidth); // Account for padding
        
        const wrapTextLines = (text: string, maxChars: number) => {
          if (maxChars < 5) return [text]; // Don't wrap if too narrow
          ...
          for (const paragraph of paragraphs) {
            ...
            // Add lines to the result, with empty line if paragraph was empty
            if (lines.length === 0) {
              allLines.push('');
            } else {
              allLines.push(...lines);
        ...
        const wrappedLines = wrapTextLines(textContent, maxCharsPerLine);
        
        return (
          <g key={element.id}>
            {/* Background rectangle like code block */}
            <rect 
              x={element.x} 
              y={element.y} 
              width={element.width} 
              height={element.height} 
              fill={element.backgroundColor || 'rgba(243, 244, 246, 0.9)'} 
              stroke={element.strokeColor || '#374151'} 
              strokeWidth={element.strokeWidth || 2} 
              rx={4} 
              ry={4}
              opacity={element.opacity}
            />
            {/* Text content with padding and wrapping */}
            {wrappedLines.map((line, i) => (
              <text 
                key={i} 
                x={element.x + 8} 
                y={element.y + 20 + (i * fontSize * 1.2)} 
                fill={element.strokeColor || '#374151'} 
                fontSize={fontSize} 
  ...
  return (
    <div ref={containerRef} className="w-full h-full bg-gray-50 overflow-auto relative" style={{ scrollbarWidth: 'thin', scrollbarColor: '#d1d5db #f9fafb' }}>
      ...
      {/* NEW TEXT INPUT - positioned outside scaled container */}
      {isTextActive && (
        <textarea
          ref={textInputRef}
          value={textInput}
          ...
          onKeyDown={(e) => {
            ...
            if (e.key === 'Enter' && !e.shiftKey) {
              e.preventDefault();
              console.log('⌨️ Enter pressed, finishing input');
              finishTextInput();
            } else if (e.key === 'Escape') {
              e.preventDefault();
              console.log('⌨️ Escape pressed, canceling input');
              setIsTextActive(false);
              setTextInput('');
              setEditingTextId(null);
            }
          ...
          onMouseDown={(e) => {
            console.log('🖱️ Textarea mouse down');
            e.stopPropagation();
          }}
          onClick={(e) => {
          ...
          onBlur={() => {
            console.log('👁️ Textarea lost focus, finishing input');
            // Only finish if we're actually losing focus to something outside the textarea
            // and not due to immediate re-rendering
            setTimeout(() => {
          ...
          onFocus={() => {
            console.log('👁️ Textarea gained focus');
          }}
          className="absolute bg-white border-2 border-blue-500 px-2 py-1 outline-none resize-none rounded"
          style={{
            left: `${(textPosition.x + 8) * zoom}px`, // Scale position with zoom
            top: `${(textPosition.y + 8) * zoom}px`,   // Scale position with zoom
            fontSize: `${fontSize * zoom}px`,          // Scale font size with zoom for better visibility
            fontFamily: fontFamily,
            color: '#374151', // Match text color
            textAlign: textAlign,
            width: `${(editingTextId ? elements.find(el => el.id === editingTextId)?.width || 180 : 180) * zoom - 16}px`, // Match text box width
            height: `${(editingTextId ? elements.find(el => el.id === editingTextId)?.height || 40 : 40) * zoom - 16}px`, // Match text box height
            zIndex: 1001,
            pointerEvents: 'auto', // Ensure it receives events
            wordWrap: 'break-word',
            whiteSpace: 'pre-wrap',
            overflow: 'auto', // Allow scrolling if content overflows
            lineHeight: `${fontSize * zoom * 1.2}px`, // Scale line height with zoom
          }}
          placeholder="Type text... (Enter to finish, Shift+Enter for new line)"
          autoFocus
        />
      )}
      ...
      <div className="relative" style={{ width: CANVAS_WIDTH, height: CANVAS_HEIGHT, transform: `scale(${zoom})`, transformOrigin: '0 0' }}>
        ...
        <svg 
          ...
          style={{ 
            ...
            pointerEvents: isTextActive ? 'none' : 'auto' // Disable mouse events when typing
          }}
        >
          <defs>
      ...
      {isTextActive && (
        <div className="fixed bottom-20 left-1/2 transform -translate-x-1/2 bg-green-600 text-white px-3 py-1 rounded text-sm z-20">
          Text Input - Press Enter to save, Shift+Enter for new line, Escape to cancel
        </div>
      )}
      {selectedTool === 'eraser' && (
        <div className="fixed bottom-20 left-1/2 transform -translate-x-1/2 bg-black/75 text-white px-3 py-1 rounded text-sm z-20">
