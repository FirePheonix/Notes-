...
export const Canvas: React.FC<CanvasProps> = ({
  ...
  const startTextEdit = useCallback((canvasX: number, canvasY: number, existingElement?: Element) => {
    // Store canvas coordinates directly for positioning within the canvas
    setTextPosition({ x: canvasX, y: canvasY });
    setTextValue(existingElement?.text || '');
    setEditingElementId(existingElement?.id || null);
  ...
  const finishTextEdit = useCallback(() => {
    ...
    if (editingElementId) {
      const newElements = elements.map(el => {
        if (el.id === editingElementId) {
          ...
          return { ...el, text: textValue, width: Math.max(maxLineLength * 8, 50), height: lines.length * 20 };
        }
        return el;
      });
      onElementsChange(newElements);
    } else {
      // Create new text element at the clicked position
      const lines = textValue.split('\n');
      const maxLineLength = Math.max(...lines.map(line => line.length));
    ...
    setEditingElementId(null);
    setTextValue('');
  }, [isTextEditing, textValue, textPosition, editingElementId, elements, onElementsChange, strokeColor, strokeWidth, strokeStyle, roughness, fontSize, fontFamily]);
  ...
  const handleMouseDown = useCallback((event: React.MouseEvent) => {
    ...
    if (isTextEditing) { finishTextEdit(); return; }
    if (selectedTool === 'eraser') { setIsErasing(true); setErasedElements(new Set()); handleErase(point); return; }
    ...
    if (selectedTool === 'hand') { setIsPanning(true); setLastPanPoint({ x: event.clientX, y: event.clientY }); return; }

    if (selectedTool === 'text') {
      const clickedTextElement = [...elements].reverse().find(el => !el.isDeleted && el.type === 'text' && isPointInElement(point, el));
      startTextEdit(clickedTextElement?.x || point.x, clickedTextElement?.y || point.y, clickedTextElement);
      return;
    }
    ...
    if (selectedTool === 'image') { fileInputRef.current?.click(); return; }
    ...
    handleErase, isTextEditing, finishTextEdit, startTextEdit, strokeWidth, strokeStyle, roughness, onElementsChange
  ]);
  ...
  const renderElement = (element: Element) => {
    ...
    const commonProps = { stroke: element.strokeColor, fill: element.backgroundColor === 'transparent' ? 'none' : element.backgroundColor, strokeWidth: element.strokeWidth, strokeDasharray, opacity: element.opacity };

    switch (element.type) {
      case 'rectangle': return <rect key={element.id} x={element.x} y={element.y} width={element.width} height={element.height} {...commonProps} />;
      case 'circle': return <ellipse key={element.id} cx={element.x + element.width / 2} cy={element.y + element.height / 2} rx={element.width / 2} ry={element.height / 2} {...commonProps} />;
      case 'diamond': return <path key={element.id} d={`M ${element.x + element.width / 2} ${element.y} L ${element.x + element.width} ${element.y + element.height / 2} L ${element.x + element.width / 2} ${element.y + element.height} L ${element.x} ${element.y + element.height / 2} Z`} {...commonProps} />;
      case 'line': return <line key={element.id} x1={element.x} y1={element.y} x2={element.x + element.width} y2={element.y + element.height} {...commonProps} />;
      case 'arrow':
        const angle = Math.atan2(element.height, element.width);
        const arrowX = element.x + element.width;
        const arrowY = element.y + element.height;
        return <g key={element.id}><line x1={element.x} y1={element.y} x2={arrowX} y2={arrowY} {...commonProps} /><path d={`M ${arrowX} ${arrowY} L ${arrowX - 15 * Math.cos(angle - Math.PI / 6)} ${arrowY - 15 * Math.sin(angle - Math.PI / 6)} M ${arrowX} ${arrowY} L ${arrowX - 15 * Math.cos(angle + Math.PI / 6)} ${arrowY - 15 * Math.sin(angle + Math.PI / 6)}`} {...commonProps} fill="none" /></g>;
      case 'draw': return <path key={element.id} d={`M ${element.points?.[0].x} ${element.points?.[0].y} ${element.points?.slice(1).map(p => `L ${p.x} ${p.y}`).join(' ')}`} {...commonProps} fill="none" />;
      case 'text': return <g key={element.id}>{(element.text || '').split('\n').map((line, i) => <text key={i} x={element.x} y={element.y + (element.fontSize || 16) + (i * 20)} fill={element.strokeColor} fontSize={element.fontSize || 16} fontFamily={element.fontFamily || 'Arial, sans-serif'} opacity={element.opacity}>{line}</text>)}</g>;
      case 'image': return <image key={element.id} x={element.x} y={element.y} width={element.width} height={element.height} href={element.imageUrl} opacity={element.opacity} preserveAspectRatio="xMidYMid meet" />;
      case 'code': return <rect key={element.id} x={element.x} y={element.y} width={element.width} height={element.height} fill="rgba(139, 92, 246, 0.1)" stroke="#8b5cf6" strokeWidth={1} strokeDasharray="4,2" />;
      default: return null;
    }
  };
  ...
  const renderSelectionHandles = (element: Element) => {
    ...
    return (
      <g key={`handles-${element.id}`}>
        ...
        {handles.map((handle, index) => <rect key={index} x={handle.x} y={handle.y} width={handleSize} height={handleSize} fill="white" stroke="#1971c2" strokeWidth={1 / zoom} style={{ cursor: `${handle.type}-resize` }} />)}
      </g>
    );
  };
  ...
  return (
    <div ref={containerRef} className="w-full h-full bg-gray-50 overflow-auto relative" style={{ scrollbarWidth: 'thin', scrollbarColor: '#d1d5db #f9fafb' }}>
      ...
      <div className="relative" style={{ width: CANVAS_WIDTH, height: CANVAS_HEIGHT, transform: `scale(${zoom})`, transformOrigin: '0 0' }}>
        {isTextEditing && (
          <div
            className="absolute border-2 border-dashed border-blue-500 bg-white bg-opacity-90"
            style={{
        ...
        <svg ref={svgRef} width={CANVAS_WIDTH} height={CANVAS_HEIGHT} className="absolute top-0 left-0" onMouseDown={handleMouseDown} style={{ cursor: getCursorStyle() }}>
          ...
          <rect width="100%" height="100%" fill="url(#grid)" /><rect width="100%" height="100%" fill="white" fillOpacity="0.9" />
          {elements.map(renderElement)}
          {elements.map(renderSelectionHandles)}
        </svg>

        {elements.filter(el => el.type === 'code' && !el.isDeleted).map(element => (
          <CodeBlock key={element.id} element={element} zoom={zoom} isSelected={selectedElements.includes(element.id)}
      ...
      {elements.length === 0 && <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none text-gray-400 text-center z-10"><p className="text-lg font-medium">🎨 Welcome to your AI-powered canvas!</p><p className="text-sm">Start drawing, or create code blocks to begin.</p><p className="text-xs mt-2 text-gray-500">Canvas size: {CANVAS_WIDTH} × {CANVAS_HEIGHT}px</p></div>}
      
      {selectedTool === 'text' && !isTextEditing && <div className="fixed bottom-20 left-1/2 transform -translate-x-1/2 bg-black/75 text-white px-3 py-1 rounded text-sm z-20">Text Mode - Click anywhere to add text</div>}
      {isTextEditing && <div className="fixed bottom-20 left-1/2 transform -translate-x-1/2 bg-green-600 text-white px-3 py-1 rounded text-sm z-20">Text Editing Mode - Type your text</div>}
      {selectedTool === 'eraser' && <div className="fixed bottom-20 left-1/2 transform -translate-x-1/2 bg-black/75 text-white px-3 py-1 rounded text-sm z-20">Eraser Mode - Click or drag to erase elements</div>}
    </div>
  );
};
